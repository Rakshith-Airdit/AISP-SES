sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "com.aisp.aispsupplierportal.controller.ObjectPage",
      {
        ...
        _oModelRead: function (entity, poNo) {
          return new Promise((resolve, reject) => {
            ...
            oModel.read(`/${entity}`, {
              ...
              error: (error) => {
                this.getView().byId("idObjectPage").setBusy(false);
                const errorMsg = JSON.parse(error.responseText).error.message
                  .value;
        ...
        _setupServiceItems: function () {
          this.getView()
            .byId("idTotalPrice")
            .setText("Total Service Sheet Value : 00");

          const selectedItemsModel =
            this.getOwnerComponent().getModel("selItemModel");
        ...
        loadFragment: function (srvType) {
          ...
          const fragmentType =
            srvType === "Planned" ? "PlannedFragment" : "UnplannedFragment";
          ...
          try {
            ...
            if (this.srvType === "Planned") {
              ...
              if (totalSrvSheetVal > orderLimit) {
                console.error(
                  `Total Service Sheet value ${totalSrvSheetVal} exceeds order amount ${orderLimit}`
                );
            ...
            this.getView()
              .byId("idTotalPrice")
              .setText(`Total Service Sheet Value : ${totalSrvSheetVal}`);
          } catch (error) {
        ...
        onInputChange: function (oEvent) {
          ...
          if (
            this.srvType === "Planned" &&
            inputSource === "PlannedSrvQtyInput"
          ) {
        ...
        _handlePlannedQuantityChange: function (oEvent) {
          ...
          if (serviceQuantity > remainingQuantity) {
            ...
            this._setInputErrorState(oInput, "");
          }
        ...
        _handleUnplannedInputChange: function (oEvent, inputSource) {
          ...
          const property =
            inputSource === "UnplannedSrvQtyInput"
              ? "SERVICE_QUANTITY"
              : "UNIT_PRICE";
          ...
          if (inputSource === "UnplannedSrvQtyInput" && value > orderLimit) {
            // Example max limit
            this._setInputErrorState(
              oInput,
              `Maximum quantity is ${orderLimit}`
            );
            this._resetInputToPreviousValue(oInput, oData, property);
            return;
          }
        ...
        _updateTotalPrice: function () {
          ...
          const orderLimit = Number(
            this.getView().getModel("poModel").getData()[0]?.Amount
          );
          let totalSrvSheetVal = 0;
        ...
        _calculateUnplannedTotal: function (aItems, orderLimit) {
          ...
          const totalSrvSheetVal =
            itemsTotal + (this._previousTotalSrvValue || 0);
        ...
        onFileSelected: function (oEvent) {
          ...
          const aAttachments =
            oAttachmentsModel.getProperty("/attachments") || [];

          if (aAttachments.length >= 1) {
            MessageBox.alert(
              "Only one attachment allowed. Please delete the existing file first."
            );
            this.byId("fileUploader").clear();
            return;
          }
          ...
          oReader.onload = function (e) {
            ...
            this.byId("attachmentsCountTitle").setText(
        ...
        onPreviewAttachment: function (oEvent) {
          const oContext = oEvent
            .getSource()
            .getBindingContext("attachmentsModel");
          const oData = oContext.getObject();
        ...
        getServicePeriodString: function () {
          ...
          const dateRange =
            oServicePeriodInput.getDateValue() &&
            oServicePeriodInput.getSecondDateValue()
              ? [
                  oServicePeriodInput.getDateValue(),
                  oServicePeriodInput.getSecondDateValue(),
                ]
          ...
          return `${formattedDate.format(
            dateRange[0]
          )} to ${formattedDate.format(dateRange[1])}`;
        },
        ...
        collectServiceItems: function () {
          ...
          for (const rowItem of aSrvItemRows) {
            ...
            if (iUnitPrice <= 0) {
              throw new Error(
                `Row ${rowItem.SR_NO}: Unit Price cannot be empty or negative number.`
              );
            }
          ...
          oTotalSrvSheetVal.totalSrvSheetVal = Number(
            oTotalSrvSheetVal.totalSrvSheetVal.toFixed(2)
          );
        ...
        _validateForm: function () {
          ...
          const aAttachments =
            oView.getModel("attachmentsModel").getProperty("/attachments") ||
            [];
            ...
            servicehead: [
              {
                ...
                SERVICE_LOCATION: oView
                  .byId("idServiceLocation")
                  .getValue()
                  .trim(),
                PERSON_RESPONSIBLE: oView
                  .byId("idPersonResponsible")
                  .getValue()
                  .trim(),
                COMPANY_CODE:
                  oHeaderData.Bukrs || oHeaderData.COMPANY_CODE || "",
                PO_NUMBER: this.poNo,
                AMOUNT: (
                  oHeaderData.Amount ||
                  oHeaderData.AMOUNT ||
        ...
        onDeletePress: function (oEvent) {
          ...
          for (let i = aSelectedItems.length - 1; i >= 0; i--) {
            ...
            const oData = oSelectedItem
              .getBindingContext("selItemModel")
              .getObject();
            const index = aItems.findIndex(
              (item) => item.SR_NO === oData.SR_NO
            );
        ...
        _validatePlannedItem: function (rowItem) {
          if (rowItem.newField === undefined || isNaN(rowItem.newField)) {
            throw new Error(
              `Row ${rowItem.SR_NO}: Service Quantity cannot be empty.`
            );
          }

          if (rowItem.SERVICE_QUANTITY === undefined) {
            throw new Error(
              `Row ${rowItem.SR_NO}: Serviced Quantity cannot be empty.`
            );
          }
          ...
          if (remainingAllowed < 0) {
            throw new Error(
              `Row ${rowItem.SR_NO}: Service Quantity (${Math.abs(
                iServicedQty
              )}) exceeds remaining ` +
        ...
        _validateUnplannedItem: function (rowItem) {
          if (!rowItem.SERVICE_NUMBER) {
            throw new Error(
              `Row ${rowItem.SR_NO}: Service Number cannot be empty.`
            );
          }
          if (rowItem.SERVICE_QUANTITY === undefined) {
            throw new Error(
          ...
          const iMaxOrderAmount = Number(
            this.getView().getModel("poModel").getProperty("/0/Amount") || 0
          );
        ...
        _buildCleanItem: function (rowItem, iUnitPrice) {
          ...
          return {
            ...
            ORDERED_QUANTITY: rowItem.ORDERED_QUANTITY
              ? Number(rowItem.ORDERED_QUANTITY)
              : 0,
        ...
        onValueHelpOkPress: function (oEvent) {
          ...
          const { ServiceNumber, ServiceText, UnitOfMesure } =
            selectedProperties;

          // Update model with selected values
          oSelItemModel.setProperty(
